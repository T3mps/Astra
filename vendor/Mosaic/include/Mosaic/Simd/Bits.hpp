...
// Mosaic::Simd (Bits) -- the SIMD *bit* toolbox: byte-match masks over 16/32-byte
// groups (the engine behind open-addressing hash tables), 128/256-bit bitmap
// logic, a hardware hash-combine, prefetch hints, and batch helpers. The
// counterpart to <Mosaic/Simd/Wide.hpp>, which owns the numeric lane vectors.
//
// Move origin: Astra Core/Simd.hpp (the canonical Starworks copy -- FlatMap /
// FlatSet / Bitmap / Archetype key off these).
//
// Ethos: SPEED-FIRST. Every operation here is exact integer/byte work whose
// result is identical on every backend by construction, so a backend may be
// chosen purely on throughput. This is deliberately NOT the Wide layer's
// determinism contract (floating-point, /fp:strict, move-verbatim) -- do not
// conflate the two.
//
// Width is EXPLICIT at every call site (MatchByteMask<Width128>, never an
// implicit "best available"): a group's size is a data-layout decision the
// container owns, not something the ISA gets to change underneath it.
//
// Alignment: the Load*/Match* entry points take ALIGNED pointers (16B for
// Width128, 32B for Width256) -- checked by MOSAIC_ASSERT below.

#include <Mosaic/Assert.hpp>     // MOSAIC_ASSERT (alignment preconditions)
#include <Mosaic/Bits.hpp>       // PopCount / CountTrailingZeros / FindFirstSet / FindLastSet
...
// ---------------------------------------------------------------------------
// Intrinsics headers for the active architecture.
// ---------------------------------------------------------------------------
#if defined(MOSAIC_ARCH_X64) || defined(MOSAIC_ARCH_X86)
    #if defined(MOSAIC_COMPILER_MSVC)
        #include <intrin.h>
        #include <nmmintrin.h>  // SSE4.2
        #if defined(MOSAIC_HAS_AVX) || defined(MOSAIC_HAS_AVX2)
...
namespace Mosaic
{
    namespace Simd
    {
        ...
        template<SimdWidth Width>
        MOSAIC_NODISCARD MOSAIC_FORCEINLINE bool IsAligned(const void* ptr) noexcept
        {
        ...
        {
            template<SimdWidth Width>
            MOSAIC_NODISCARD inline constexpr bool HasWidth() noexcept
            {
                if constexpr (std::is_same_v<Width, Width128>)
                {
#if defined(MOSAIC_HAS_SSE2) || defined(MOSAIC_HAS_NEON)
                    return true;
#else
                    return false;
#endif
            ...
            template<SimdWidth Width>
            MOSAIC_NODISCARD inline constexpr bool HasFallback() noexcept
            {
            ...
            // Best width for a given data size. Informational only -- callers must
            // still specify the width explicitly.
            MOSAIC_NODISCARD inline constexpr size_t SuggestedWidthForSize(size_t data_size) noexcept
            {
        ...
        namespace Ops
        {
            // The scalar bit scans live in Mosaic::Bits (they need no SIMD); they are
            // re-exported here because every byte-match result feeds straight into one.
            using ::Mosaic::Bits::CountTrailingZeros;
            using ::Mosaic::Bits::FindFirstSet;
            using ::Mosaic::Bits::FindLastSet;
            using ::Mosaic::Bits::PopCount;

            // ============== 128-bit implementations ==============
            namespace Detail128
            {
                MOSAIC_FORCEINLINE uint16_t MatchByteMask_SSE(const void* data, uint8_t value) noexcept
                {
                ...
                MOSAIC_FORCEINLINE uint16_t MatchByteMask_NEON(const void* data, uint8_t value) noexcept
                {
                ...
                MOSAIC_FORCEINLINE uint16_t MatchByteMask_Scalar(const void* data, uint8_t value) noexcept
                {
                    uint16_t mask = 0;
                ...
                MOSAIC_FORCEINLINE uint16_t MatchEitherByteMask_SSE(const void* data, uint8_t val1, uint8_t val2) noexcept
                {
#if defined(MOSAIC_HAS_SSE2)
                    const __m128i group = _mm_load_si128(static_cast<const __m128i*>(data));
                ...
                MOSAIC_FORCEINLINE uint16_t MatchEitherByteMask_NEON(const void* data, uint8_t val1, uint8_t val2) noexcept
                {
#if defined(MOSAIC_HAS_NEON)
                    const uint8x16_t group = vld1q_u8(static_cast<const uint8_t*>(data));
                ...
                MOSAIC_FORCEINLINE uint16_t MatchEitherByteMask_Scalar(const void* data, uint8_t val1, uint8_t val2) noexcept
                {
                    uint16_t mask = 0;
                    const uint8_t* bytes = static_cast<const uint8_t*>(data);
                    for (int i = 0; i < 16; ++i)
            ...
            namespace Detail256
            {
                MOSAIC_FORCEINLINE uint32_t MatchByteMask_AVX(const void* data, uint8_t value) noexcept
                {
#if defined(MOSAIC_HAS_AVX2)
                ...
                MOSAIC_FORCEINLINE uint32_t MatchByteMask_Fallback(const void* data, uint8_t value) noexcept
                {
#if defined(MOSAIC_HAS_SSE2)
                ...
                MOSAIC_FORCEINLINE uint32_t MatchEitherByteMask_AVX(const void* data, uint8_t val1, uint8_t val2) noexcept
                {
#if defined(MOSAIC_HAS_AVX2)
                    const __m256i group = _mm256_load_si256(static_cast<const __m256i*>(data));
                ...
                MOSAIC_FORCEINLINE uint32_t MatchEitherByteMask_Fallback(const void* data, uint8_t val1, uint8_t val2) noexcept
                {
#if defined(MOSAIC_HAS_SSE2)
                    // Optimized 2x128 with better scheduling
                    const uint8_t* ptr = static_cast<const uint8_t*>(data);
            ...
            template<SimdWidth Width>
            MOSAIC_FORCEINLINE auto MatchByteMask(const void* data, uint8_t value) noexcept -> typename WidthTraits<Width>::MaskType
            {
                MOSAIC_ASSERT(IsAligned<Width>(data), "Data must be aligned for SIMD operations");
            ...
            template<SimdWidth Width>
            MOSAIC_FORCEINLINE auto MatchEitherByteMask(const void* data, uint8_t val1, uint8_t val2) noexcept -> typename WidthTraits<Width>::MaskType
            {
                MOSAIC_ASSERT(IsAligned<Width>(data), "Data must be aligned for SIMD operations");
            ...
            MOSAIC_FORCEINLINE Int128 Load128(const void* ptr) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE Int128 And128(Int128 a, Int128 b) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE Int128 Or128(Int128 a, Int128 b) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE void Store128(void* ptr, Int128 value) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE Int128 CompareEqual128(Int128 a, Int128 b) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE bool TestEqual128(Int128 a, Int128 b) noexcept
            {
#if defined(MOSAIC_HAS_SSE2)
            ...
            MOSAIC_FORCEINLINE bool TestSubset128(Int128 a, Int128 b) noexcept
            {
            ...
            MOSAIC_FORCEINLINE Int256 Load256(const void* ptr) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            MOSAIC_FORCEINLINE void Store256(void* ptr, Int256 value) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            MOSAIC_FORCEINLINE Int256 And256(Int256 a, Int256 b) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            MOSAIC_FORCEINLINE Int256 Or256(Int256 a, Int256 b) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            MOSAIC_FORCEINLINE Int256 CompareEqual256(Int256 a, Int256 b) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            MOSAIC_FORCEINLINE bool TestEqual256(Int256 a, Int256 b) noexcept
            {
#if defined(MOSAIC_HAS_AVX2)
            ...
            // Test if (a & b) == a (i.e., a is subset of b)
            MOSAIC_FORCEINLINE bool TestSubset256(Int256 a, Int256 b) noexcept
            {
                Int256 result = And256(a, b);
                return TestEqual256(result, a);
            }

            // Hardware CRC32 or high-quality fallback. FAST but ISA-DEPENDENT: the
            // value differs between a CRC32C-capable target and the fallback, so it
            // must never reach a persisted format or a wire protocol -- use
            // PortableHashCombine there.
            MOSAIC_FORCEINLINE uint64_t HashCombine(uint64_t seed, uint64_t value) noexcept
            {
#if defined(MOSAIC_HAS_SSE42)
                return _mm_crc32_u64(seed, value);
#elif defined(MOSAIC_HAS_ARM_CRC32)
            ...
            // ISA-independent 64-bit mix (MurmurHash3 finalizer). Identical output on
            // every platform -- what a persisted checksum must use.
            MOSAIC_FORCEINLINE uint64_t PortableHashCombine(uint64_t seed, uint64_t value) noexcept
            {
                uint64_t h = seed ^ value;
                h ^= h >> 33;
                h *= 0xff51afd7ed558ccdULL;
            ...
            MOSAIC_FORCEINLINE void PrefetchRead(const void* ptr, PrefetchHint hint = PrefetchHint::T0) noexcept
            {
#if defined(MOSAIC_ARCH_X64) || defined(MOSAIC_ARCH_X86)
    #if defined(MOSAIC_COMPILER_MSVC)
                switch (hint)
            ...
            MOSAIC_FORCEINLINE void PrefetchT0(const void* ptr) noexcept { PrefetchRead(ptr, PrefetchHint::T0); }
            MOSAIC_FORCEINLINE void PrefetchT1(const void* ptr) noexcept { PrefetchRead(ptr, PrefetchHint::T1); }
            MOSAIC_FORCEINLINE void PrefetchT2(const void* ptr) noexcept { PrefetchRead(ptr, PrefetchHint::T2); }
            MOSAIC_FORCEINLINE void PrefetchNTA(const void* ptr) noexcept { PrefetchRead(ptr, PrefetchHint::NTA); }

            // ============== Batch Operations ==============

            // Process multiple groups in a batch, prefetching the next batch as it goes.
            ...
            struct BatchOps
            {
                ...
                static MOSAIC_FORCEINLINE void MatchByteMaskBatch(
                    const void* data,
                    uint8_t value,
                    MaskType* results,
                ...
                static MOSAIC_FORCEINLINE void MatchEitherByteMaskBatch(
                    const void* data,
                    uint8_t val1,
                    uint8_t val2,
                    MaskType* results,
                ...
                // Byte offset of the first matching byte across `count` groups, or -1.
                static MOSAIC_FORCEINLINE int FindFirstMatchInBatch(
                    const void* data,
                    uint8_t value,
                    size_t count) noexcept
                {
