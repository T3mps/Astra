...
The `SystemTraits` specialization parses the pack, extracts `Reads`/`Writes` as today, and surfaces `static constexpr bool RequiresExclusive` (true iff an `Exclusive` tag is present). The scheduler reads that flag; a direct `static constexpr bool RequiresExclusive = true;` member is *also* sniffed as an ergonomic alias (covers Mass-familiar users and systems not using `SystemTraits` inheritance). Mechanically, `Exclusive` reuses the path no-trait systems already take: it conflicts with every group and its own group accepts no members ⇒ it lands alone. Adjacent groups still parallelize; because groups are sequential barriers, nothing runs concurrently with an exclusive system.

**Enforcement tripwire (Debug only, zero Release cost).** The executor snapshots `ArchetypeManager::m_structuralChangeCounter` (the atomic that already exists at `ArchetypeManager.hpp:1400` and that `View` already uses this exact way at `View.hpp:108`) immediately before dispatching a *multi-member* group and immediately after its barrier. If it changed, some grouped system performed an undeclared structural change → `ASTRA_ASSERT` naming the group. Deferred changes (via `CommandBuffer`) don't bump the counter during the group, so the safe pattern never false-trips — the assert *rewards* deferral, which is precisely B2's model.

**Named limitation (documented, not hidden):** the counter tripwire catches *structural* violations (the corruption-causing case). It does **not** catch a system reading/writing a component it didn't declare but another grouped system owns (a mask-completeness breach). The contract is "declare *all* access"; TSan is the tool for that class (unavailable on MSVC — see §6). B2's access-set expansion is the real fix.
...
2. **Downgrade the flag's advertised role (I1).** Rewrite the comment: the flag does **not** provide mutual exclusion against external threads. It exists to make the one practical mistake safe — a system, on a worker, calling `RemoveSystem`/`AddSystem` mid-frame, which no-ops via the dispatch happens-before (now guaranteed by the I4 fix). Keep the `ASTRA_ASSERT(!IsExecuting()) + if (IsExecuting()) return …;` shape: assert in Debug, graceful in Release.
