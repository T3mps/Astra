...
3. **H4 mechanism = replace `std::any` with an Astra-native `TypeID`-tagged carrier** (`AnyValue`), NOT "verify `std::any` survives `-fno-rtti` + guard." Consistent with the typed `FieldInfo::Get`/`Set` path that already checks `TypeID<T>::Hash()`. **Justification (corrected after research — see "Research validation" below): the win is determinism + cross-module correctness + SBO control + a no-throw API, NOT raw compare speed** (a `uint64` hash compare is a measured wash vs `std::any`'s manager-pointer compare; both are ~0.65 ns). Specifically: (a) `std::any` under `-fno-rtti` is implementation-defined and carries a *silent* cross-DSO `any_cast` false-`nullptr` hazard on libstdc++/libc++; (b) a strict console config (`_HAS_STATIC_RTTI=0`, beyond `/GR-`) makes MSVC's `<any>` a hard compile error, so any TU including reflection fails to build; (c) `std::any`'s inline-buffer size is vendor-dependent (libstdc++ = 8 bytes → a 12-byte `Vec3` heap-allocates), whereas `AnyValue` makes the SBO threshold an explicit, testable contract.
...
- **Storage:** a fixed inline small-buffer (SBO) with heap fallback for larger/over-aligned `T`. Mirror the **`Delegate` SBO + function-pointer-manager idiom** (and `ComponentDescriptor`'s copy/move/destruct thunks) — NOT `AlignedStorage` (that is a compile-time two-type union, unsuitable for a runtime-typed box). Honor `alignof(T)` for the heap path (reuse the aligned-`new` idiom from the just-landed `SmallVector::Allocate`).
  - **SBO size — deliberate, not inherited:** pick the inline buffer to keep Astra's *common reflected field types* allocation-free. Do NOT inherit `std::any`'s vendor-dependent ceiling (libstdc++ inlines only 8 bytes → a 12-byte `Vec3` heap-allocates on GCC). A **16-byte / 8-aligned** buffer (EnTT's `entt::any` default = "two doubles"; Folly `Poly` = same) covers `int`/`float`/`bool`/pointer/`Vec3`/`Vec4`; size up (e.g. to a `mat4`) only if profiling the actual reflected types shows larger fields are common. The SBO/heap boundary is an explicit, unit-tested contract (both branches exercised), unlike `std::any`'s best-effort behavior.
...
### Result
No `std::any`/`typeid`/`type_info` (and no `<any>` include) anywhere in `Reflection/` — RTTI-free by construction, deterministic across toolchains, cross-DSO-correct, and consistent with the existing `TypeID`-based typed accessors. The dynamic path is a modest constant-factor lighter than `std::any` (from dropping `std::any`'s generality/SBO overhead), but the headline win is correctness + portability, not speed.

### Keep the zero-copy typed path as the documented fast path
The benchmark ranking (below) confirms the fastest RTTI-free dynamic access is the **already-existing** typed `FieldInfo::GetPtr<T>` / `Get<T>` / `Set<T>` (a `TypeID<T>::Hash()` guard + a pointer or single copy — no box, no allocation). `AnyValue` is only for the "type is erased at the call site" use case (generic serializers, editor property grids). This work must NOT push callers onto the box when they know `T`; keep the typed accessors as the documented fast path. A **non-owning typed view** (`{const void*, uint64 typeHash}`, zero-copy) would be faster still for hot getters but is DEFERRED — it re-introduces exactly the pointer-lifetime hazard that bit `ResourceStorage` when `FlatMap` proved non-pointer-stable ([[astra-flatmap-pointer-stability]]); value semantics (matching `std::any` and `entt::any`'s default) is correct for v1.

### Research validation (2026-07-21, three parallel investigations — stdlib source, MSVC microbenchmarks, prior-art survey)
- **Best-practice match:** the `AnyValue` design is an independent re-derivation of `entt::any`/`entt::meta_any` (SBO+heap, name-hash `id_type` tag, function-pointer manager, `try_cast`→null) — the most directly comparable RTTI-free game-ECS reflection prior art. RTTR (`rttr::variant`) is the same shape with a name-registered integer id, built expressly to avoid `typeid` across DLL boundaries. Reusing Astra's `TypeID::Hash()` rather than a second identity scheme is confirmed correct (EnTT funnels every identity check through its one `type_hash`; Astra already hardened that hash's collision behavior in Theme E).
- **Perf reality (measured on this MSVC toolchain):** `dynamic_cast` ~18× baseline, `typeid`/`type_info::operator==` ~9× (and degrades to `strcmp` across DSOs), `std::any_cast` pointer-form ~2.6× (1.5 ns), a `uint64`-hash or function-pointer tag compare ~1.1× (0.65 ns, indistinguishable from each other). Conclusion: the type-check choice is a wash once you leave RTTI; the value comes from avoiding `std::any`'s generality overhead + SBO control, not the compare.
- **Two hazards that motivate the removal:** (1) under `-fno-rtti`, libstdc++/libc++ compile out `any_cast`'s `typeid` fallback, so `any_cast<T>(&v)` can return a *false* `nullptr` for the correct `T` across a plugin/DLL boundary (GCC-maintainer-documented); (2) a strict console config `_HAS_STATIC_RTTI=0` (beyond `/GR-`) makes MSVC's `<any>` a hard compile error, breaking every TU that includes reflection.
- **Accepted, pre-existing limitation (not introduced here):** the name-hash identity can theoretically collide (pigeonhole) or conflate identically-named anon-namespace/cross-DLL types — the EnTT-documented accepted limitation, already covered for Astra by Theme E's `TypeIdentity` collision detection. `AnyValue` inherits that guarantee by reusing `TypeID`, adds nothing new.
