    ...
    public:
        ...
        std::pair<iterator, bool> Emplace(K&& key, Args&&... args)
        {
            ...
            while (probes++ < m_capacity) ASTRA_LIKELY
            {
                ...
                // chain, so the key may still exist in a later group. Inserting
                // at the first tombstone before the duplicate scan reaches an
                // EMPTY slot would double-insert the key.
                ...
                // An EMPTY slot terminates the probe chain: the key cannot live
